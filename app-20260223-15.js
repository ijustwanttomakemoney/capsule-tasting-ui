...
async function autoLoadCatalogIfEmpty(){
  // If the user has only the tiny seed catalog, auto-load Nespresso catalog once.
  ...
  try{
    ...
    // Silent fail: user can still click "Load Nespresso catalog"
    return false;
  }
}
...
// Initial render (auto-load catalog on first run)
(async ()=>{
  await autoLoadCatalogIfEmpty();
  render();
})();
