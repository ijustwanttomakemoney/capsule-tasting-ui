...
const detailHero = document.querySelector('.detail__hero');
...
function cardEl(c){
  ...
  const imgUrl = nespressoImageUrl(c);

  el.innerHTML = `
    <div class="card__hero">
      ${imgUrl ? `<img class="card__img" alt="${escapeHtml(c.name)} capsule" loading="lazy" src="${imgUrl}" onerror="this.onerror=null; this.src='${FALLBACK_CAPSULE_SVG}'">` : `<img class="card__img" alt="${escapeHtml(c.name)} capsule" loading="lazy" src="${FALLBACK_CAPSULE_SVG}">`}
...
const FALLBACK_CAPSULE_SVG = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(`
<svg xmlns='http://www.w3.org/2000/svg' width='360' height='360' viewBox='0 0 360 360'>
  <defs>
    ...
    <radialGradient id='r' cx='50%' cy='30%' r='70%'>
      <stop offset='0' stop-color='#caa35a' stop-opacity='.35'/>
      <stop offset='1' stop-color='#caa35a' stop-opacity='0'/>
    </radialGradient>
  </defs>
  <rect x='0' y='0' width='360' height='360' rx='28' fill='url(#g)'/>
  <circle cx='90' cy='90' r='140' fill='url(#r)'/>
  <g transform='translate(0,10)'>
    <path d='M180 60c60 0 100 24 100 56 0 14-8 28-22 40-12 10-26 18-42 22v26c0 14-16 24-36 24s-36-10-36-24v-26c-16-4-30-12-42-22-14-12-22-26-22-40 0-32 40-56 100-56z' fill='rgba(255,255,255,.10)' stroke='rgba(255,255,255,.18)' stroke-width='3'/>
    <path d='M144 178c12 4 24 6 36 6s24-2 36-6v26c0 8-12 14-36 14s-36-6-36-14v-26z' fill='rgba(255,255,255,.06)'/>
    <ellipse cx='180' cy='110' rx='80' ry='28' fill='rgba(0,0,0,.18)'/>
  </g>
  <text x='180' y='320' text-anchor='middle' font-family='Inter, Arial' font-size='18' fill='rgba(255,255,255,.55)'>Nespresso</text>
</svg>
`)}`;

function nespressoImageUrl(c){
  // Option B: hotlink to Nespresso CDN images (fast, but may break if Nespresso changes paths).
  // This uses a common pattern seen on Nespresso FR product pages.
  const idPart = String(c?.id||'').split(':')[1] || '';
  if(!idPart) return null;
  const slug = idPart.toLowerCase();
  return `https://www.nespresso.com/shared_res/agility/global/coffees/ol/sku-main-info-product/${encodeURIComponent(slug)}_2x.png?impolicy=small&imwidth=600&imdensity=1`;
}
...
function openDetail(c){
  ...
  // hero image
  if(detailHero){
    const imgUrl = nespressoImageUrl(c) || FALLBACK_CAPSULE_SVG;
    let img = detailHero.querySelector('img.detail__img');
    if(!img){
      img = document.createElement('img');
      img.className = 'detail__img';
      img.loading = 'lazy';
      img.alt = `${c.name} capsule`;
      detailHero.appendChild(img);
    }
    img.onerror = ()=>{ img.onerror = null; img.src = FALLBACK_CAPSULE_SVG; };
    img.src = imgUrl;
  }
    ...
    for(const t of list){
      ...
      row.innerHTML = `
        <div class="tastingItem__left">
          ...
          ${t.notes ? `<div class="tastingItem__note">${escapeHtml(t.notes)}</div>` : ''}
        </div>
